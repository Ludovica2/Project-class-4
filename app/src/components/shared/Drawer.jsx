...
const Drawer = ({ children, type, showNewBtn = false, newBtnLabel = null , onNewClick = () => { } }) => {
    ...
    return (
        <div className={`flex justify-end absolute top-0 left-0 w-screen h-screen z-50 bg-black bg-opacity-60 transition-all ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
            <div className={`lg:w-[450px] w-screen h-screen overflow-hidden absolute right-0 top-0 bg-slate-50 transition-all dark:bg-bg_dark ${isOpen ? "mr-[0]" : "lg:mr-[-450px] mr-[-100%]"}`}>
                <div className="flex justify-between items-center px-4 py-2">
                    <div className="flex gap-4 items-center">
                        {
                            showNewBtn && (
                                <PopUpModal isOpen={isOpenModalReview} setIsOpen={setIsOpenModalReview} title={dictionary.review.ADD} sizeModal={"lg"}
                                    showBtn={(openModal) => {
                                        return <button className="btn" onClick={() => openModal(true)}>
                                            {newBtnLabel}
                                        </button>;
                                    }}
                                >
                                    {
                                        <form onSubmit={handleRatingSubmit}>
                                            ...
                                            <div className="flex flex-col mt-6">
                                                <label htmlFor="content" className="text-dark">{dictionary.review.REVIEW}</label>
                    ...
                    <button className="cursor-pointer" onClick={() => setIsOpen(false)}>
                        <i className="fa-solid fa-xmark p-4 text-[30px] dark:text-slate-100"></i>
                    </button>
                </div>
